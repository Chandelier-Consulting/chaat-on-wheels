...
export default function FoodCateringPage() {
  return (
    <main className="bg-night text-white">
      ...
      <section className="section-shell grid min-h-screen items-center gap-10 pb-20 pt-32 lg:grid-cols-[0.88fr_1.12fr]">
        <div>
          <p className="label-hero text-saffron">Catering</p>
          <h1 className="display-section mt-5 font-display font-black">Vegetarian trays with street-food energy.</h1>
          <p className="mt-6 max-w-xl text-lg font-semibold leading-8 text-white/70">
            Bring chaat, pav, lassi, chai, and sweets to offices, birthdays, family nights, and weekend gatherings.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <a className="rounded-full bg-saffron px-6 py-4 text-sm font-black text-ink" href="tel:+16696498039">
              Call catering
            </a>
            <a className="rounded-full border border-white/20 px-6 py-4 text-sm font-black" href={links.order} target="_blank" rel="noreferrer">
              Order online
            </a>
          </div>
        </div>
